...
| 18 | AI 关键帧抽取 | 无 | AI 转写时模型顺带返回关键时刻列表（重要性排序），插件用 video+canvas seek 抽帧存 Media/{note_id}/，AI 小节嵌缩略图+时间戳；ZCode 轨道用 ffmpeg；默认关、数量可调 | ⏳ M4（ADR-016） |
| 19 | 视频下载开关 | 可下载 | **默认关闭**的 `downloadVideos` 开关（应对磁盘空间不足）；开启时视频落盘 Media/{note_id}/，关闭时正文仅记链接 | ✅ 2026-09-17（M3） |
